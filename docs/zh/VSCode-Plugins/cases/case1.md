...
2. 在 **Packages** 视图安装 `gnu-plct-xthead`（如需模拟器可安装 `qemu-user-riscv-upstream`）。
...
5. 打开 VS Code 终端（继承激活环境的 PATH），进入 `coremark` 目录执行构建：

```bash
make PORT_DIR=linux64 link
```

6. 将生成的可执行程序传输到 LicheePi 4A 并运行（可用 `scp`、串口、NFS 等方式）。

## 备注
- 如需在本机模拟运行，请在选择合适的 QEMU 模拟器并在终端运行。
