# Milkv Duo：使用插件完成构建与调试（riscv64-unknown-linux-musl-bin）

本示例参考 IDE 文档，使用插件完成工具链安装与虚拟环境创建，并在 VS Code 内配合 SSH/gdbserver 进行运行与调试。原 IDE 操作与示例 Makefile 见 [Milkv Duo IDE 开发流程](../../IDE/milkv-duo-ide.md)。

## 步骤

1. 打开 VS Code，点击侧边栏 **Ruyi** 图标进入插件主页。
2. 在 **Packages** 视图安装 `gnu-milkv-milkv-duo-musl-bin`（与 IDE 文档示例一致）。
...
- 若需更完整的 IDE 配置与演示，请直接参考 [Milkv Duo IDE 开发流程](../../IDE/milkv-duo-ide.md)。